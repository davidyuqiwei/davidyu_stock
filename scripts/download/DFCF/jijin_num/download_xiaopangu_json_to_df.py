...
if __name__ == "__main__":
    columns = ["id","stock_index","stock_name","jijin_num"]
    date_type = "day"
    ...
    df2['day'] = now_date
    df2['jijin_date'] = [x.split("(")[1].replace(")","").replace("/","-") for x in df2.jijin_num.tolist()]
    df2['jijin_num'] = [x.split("(")[0] for x in df2.jijin_num.tolist()]
    print(df2.tail())
    df2.to_csv("jijin_num.csv",index=0)
