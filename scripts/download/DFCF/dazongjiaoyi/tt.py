...
def json_to_df(filename):
    ...
    for i in range(1,len(all_json)):
        ...
        df1 = df1.append(df2)
    df1['stock_date'] = [ x[0:10] for x in df1.TDATE.tolist()]
    df1['day'] = [ x[0:10] for x in df1.TDATE.tolist()]
    return df1
if __name__ == "__main__":
    df1 = json_to_df("test.txt")
    now_date,_ = get_the_datetime()
    save_name = "dazongjiaoyi_"+now_date+".csv"
    df1.to_csv(save_name,index=0)
