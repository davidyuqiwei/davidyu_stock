...
def json_to_df(file_in):
    df_len,all_keys,raw_json = load_the_json("shidagudong.txt")
    #print(raw_json)
    json_string_raw = raw_json.get(all_keys[0])
    list_string = [x.split(",") for x in json_string_raw] 
    df1 = pd.DataFrame(list_string,columns = ['id','stock_index','stock_name','total_ratio'])
    ...
    file_name = "SDGD_ratio_"+date_month+".csv"
    df2.to_csv(file_name,index=0)
if __name__ == "__main__":
    file_in = "shidagudong.txt"
    json_to_df(file_in)
