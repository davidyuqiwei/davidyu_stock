...
from functions.stock_feature.stock_make_kdj_feature import *
...
def make_data(stock,feature_list):
    df2 = stock.dropna()[history_days+10:]
    #print(df2.shape)
    df_raw = df2.dropna()
    x_col_key = ["adj_close","high","low","open","close","mv","HISTORY","vol"]+feature_list
    x_col_index = xColIndex(df_raw,x_col_key)
    y_col_key = ["FUTURE"]
    y_col_index = xColIndex(df_raw,y_col_key)
    df_model = df_raw.iloc[:,x_col_index+y_col_index]
    x_col_index = xColIndex(df_model,x_col_key)
    y_col_key = ["FUTURE"]
    y_col_index = xColIndex(df_model,y_col_key)
    return df_raw,df_model,x_col_index,y_col_index



def load_the_data(predict_days,history_days,future_predict_days,predict_col):
    ...
    df_feature = make_stock_feature(df1)
    df1 = make_history_price(df1,history_days,"close")  
    df1 = make_future_price(df1,future_predict_days,predict_col)
    stock = pd.merge(df1,df_feature)
    #stock = DF_to_StockDataFrame(df1)
    #feature_list = ['kdjk','kdjd','kdjj','macdh','rsi_6','wr_6','wr_10','wr_20',
    #            'rsi_12','rsi_24']
    feature_list = ['kdjk','kdjd','kdjj','macdh','rsi','wr']
    #stock = df1
    #stock[feature_list]
    
    return make_data(stock,feature_list)
...
def train_model(df_raw,df_model,x_col_index,y_col_index):
    ...
    model.fit(x_train, y_train, epochs=20, batch_size=200, verbose=2,validation_split=0.1)
    ...
    df_raw["stock_date"] = df_raw.index
...
if __name__ == "__main__":
    predict_days = 7
    history_days = 30
    future_predict_days = 1
    predict_col = "close"
    df_raw,df_model,x_col_index,y_col_index = load_the_data(predict_days,history_days,future_predict_days,predict_col)
    df1 = train_model(df_raw,df_model,x_col_index,y_col_index)
    file_name = "test_"+predict_col+".csv"
