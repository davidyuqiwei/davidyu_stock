...
def make_history_price(df1,history_days,col_in):
    '''
    make shift days price 
    @return close1,close2,close3   -->  1day before, 2day before, 3day before
    '''
    for i in range(1,history_days):
        col_name = "HISTORY"+str(i)
        df1[col_name] = df1[col_in].shift(i)
    return df1

def make_future_price(df1,history_days,col_in):
    '''
    make shift days price 
    @return close1,close2,close3   -->  1day before, 2day before, 3day before
    '''
    for i in range(1,history_days+1):
        ...
        df1[col_name] = df1[col_in].shift(i*-1)
    return df1
def make_future_price_up(df1,history_days,col):
    '''
    make shift days price 
    @return close1,close2,close3   -->  1day before, 2day before, 3day before
    '''
    for i in range(1,history_days+1):
        col_name = "FUTURE"+str(i)
        df1[col_name] = df1[col].shift(i*-1)
    return df1
