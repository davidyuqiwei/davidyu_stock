...
def get_train_test(stock_index,pred_days):
    input_data1,feature_columns = get_input_date(stock_index="601398",pred_days=1)
    ...
    a2.DF["slope_cls"][a2.DF["slope_cls"]>0]=1
    a2.DF["slope_cls"][a2.DF["slope_cls"]<=0]=0
    ## predict data
    ...
    pred_df_raw.round(3).to_csv("pred_df.csv",index=0)
    # train, test data
