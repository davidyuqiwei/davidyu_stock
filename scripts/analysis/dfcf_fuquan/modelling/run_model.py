...
#feature_columns = ["rsi_6", "rsi_12", "rsi_24", "kdjk", "kdjd", "kdjj", "macdh", "boll_ratio",
#        "mv_avg_3_5","mv_avg_3_8","mv_avg_3_13","mv_avg_5_8"]
#feature_columns = ["rsi_6", "rsi_12", "rsi_24", "kdjk", "kdjd", "kdjj", "macdh", "boll_ub", "boll_lb", "boll_ratio"]
...
model_run = 'vote'  ## xgb,lgb,randomF
## xgboost
...
if model_run == 'xgb':
    ...
    xgboost_model = dd.get("model").save_model('xgb_index_base.model')
...
if model_run == "LR":
    ...
    pred_train = [x[1] for x in bst.predict_proba(X_train)]
    df_pred = pd.DataFrame(pred_train)
    df_pred["y_train"]=y_train
    df_pred.columns = ["y_pred","y_train"]
