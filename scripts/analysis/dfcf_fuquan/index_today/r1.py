...
def make_test_data_stock(df3,feature_columns,stock_index):
    df4 = df3.reset_index(drop=True)
    df4 = df4.dropna().round(3).drop_duplicates()
    df5 = df4[df4["stock_index"]==stock_index]
    df6 = df5.reset_index(drop=True)
    feature_name = feature_columns
    df7 = df6[["stock_index","dt"]+feature_name]
    #-----------------------------------------------#
...
df7,test_data = make_test_data(df5,feature_columns,"2020-12-16")
#df7,test_data = make_test_data_stock(df5,feature_columns,413)
