...
object combine_data {
    ...
    */
    def combine_stock_index( start_date:String,end_date:String,table:String,data_count:Int,col_in:String):DataFrame= {
        ...
        val sql_in=" select  "+col_in+" from diff_ratio1 order by diff_ratio desc"
        val diff_ratio_out =  sqlContext.sql(sql_in)

	    diff_ratio_out
    }
    def out_df( start_date:String,end_date:String,table:String,date_count:Int,col_in:String):DataFrame= {
	    ...
	    val df_owner_ror = df_owner.filter($"dt">=start_date).filter($"dt"<=end_date).join(diff_ratio,
	        diff_ratio("code") === df_owner("stock_index"),"left").select("owner_name",
            "stock_index","diff_ratio","dt","name")
	    val df_out = df_owner_ror.groupBy("owner_name").agg(avg("diff_ratio").alias("avg_diff"),
            count("diff_ratio").alias("count")).orderBy(desc("avg_diff")).filter($"count">10)
        df_out
    }
}
