...
if __name__ == "__main__":
    now_date,now_date_time = get_the_datetime()
    now_date = "2020-09-07"
    df1 = load_data("2020-09-07")
    yeji_status = df1['yeji_predict'].unique().tolist()
    df3 = df1[(df1["yeji_predict"]=='业绩预增')&(df1["date"]==now_date)]
    df2 = df1[(df1["yeji_predict"]=='业绩大幅上升')&(df1["date"]==now_date)]
    out_file = "yeji_out_"+now_date+".csv"
    df3.to_csv(out_file,index=0)
    df2.to_csv(out_file,index=0,mode="a")
