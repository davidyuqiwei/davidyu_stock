...
from functions.day_history.dayHistoryFeatureCommon import dayHistoryFeatureCommon
...
for i in stk_index_list:
    if i[0:2] == '60' or i[0] == '00':
        try:
            ...
            history_days = 10
            dayHistoryFeatureCommon.make_history_price(df_kdj_macd,'macd',history_days)
            stock_date = df_kdj_macd['stock_date'].tail(1).values[0]
            jjj = np.round(df_kdj_macd['kdjj'].tail(1).values[0],3)
