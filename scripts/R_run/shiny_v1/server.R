...
server <- function(input, output,session) {
  ...
  # 2. Its output type is a plot
    ...
    output$table_owner_change<- DT::renderDataTable({
        #source("task_important_owner.r")
        data_out = read.csv("/home/davidyu/stock/scripts/davidyu_stock/scripts/download/all_owner/download_data/data_out2.csv")
        print("-----------")
    ...
    output$pl_dadan <- renderPlotly({
        ...
        if(v$data == 1){
            #y = data1[,y_col]
	        source('task_dadan.r')
            data1=data_dadan(input)
            rt = plot_dadan(data1)
            subplot(rt$p1,rt$p2,rt$p3,rt$p4,rt$p5,nrows=5,margin=0.03,shareX=TRUE) %>% layout(title="大单流入(单位:百万元)",height=800,margin=10)
        }
    })
