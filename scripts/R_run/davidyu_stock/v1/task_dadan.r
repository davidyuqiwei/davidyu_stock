...
data_dadan=function(input){
    ...
    data_file = paste0("../../data/dadan_dfcf/",stock_index_in,".csv")
    ...
    stock_name = as.character(data2$stock_name[1])
    source('functions.r')
    data1 = col_to_float(data2,trans_columns)
    data1 = filter(data1,dt>=start_date,dt<=end_date)
    data1 = data1[order(data1$dt),]
    return(list(data=data1,stock_name=stock_name))
}
